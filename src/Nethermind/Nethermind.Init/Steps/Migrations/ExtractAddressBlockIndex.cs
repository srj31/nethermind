...
namespace Nethermind.Init.Steps.Migrations
{
    public class ExtractAddressBlockIndex : IDatabaseMigration
    {
        ...
        private readonly string rootDirectory = @"C:\Users\merto\Programming\customInde8";
        ...
        private const int BlockBatchSize = 100;
        ...
        private void RunMigration(CancellationToken token)
        {
            ...
            try
            {
                ...
                GetBlockBodiesForMigration(token).AsParallel().WithDegreeOfParallelism(parallelism).ForAll(item =>
                {
                    ...
                    lock (flushLock)
                    {
                        if (++blocksProcessedSinceLastFlush >= BlockBatchSize)
                        {
                            FlushToDisk().Wait();
                        }
                    }
        ...
        private async Task FlushToDisk()
        {
            ...
            try
            {
                ...
                {
                    string keyAsSpan = kvp.Key.Substring(2).TrimStart('0');
                    keyAsSpan = keyAsSpan == string.Empty ? "0" : keyAsSpan;
                    ...
                    tasks.Add(WriteBlockNumsToFileAsync(blockNumbers, filePath));
                }
        ...
        private async Task WriteBlockNumsToFileAsync(ImmutableSortedSet<int> blockNumbers, string filePath)
        {
            try
            {
                ...
                {
                    data = BitConverter.GetBytes(blockNumber);
        ...
        private void ProcessFileBatch(Queue<string> fileQueue)
        {
            while (fileQueue.Count > 0)
            {
                ...
                {
                    _logger.Error($"Error occurred during file encoding for file {file}:", ex);
                }
            }
        }
